...
# Modified as needed: Nur, 03/15/2016
...
class NewRuns(object): #Will look for new runs in the swap area, fill the information about them
    def __init__(self):#Set up dictionaries, nonsense values if not filled
        self._Modes= {"numip":"NuMI Beam", 
                      "numib":"NuMI Beam",
                      "numil":"NuMI Beam",
                      "linjc":"Light Injection",
                      "pdstl":"Light Injection"} 
        self._Configs={"numip":"Beam settings",
                       "numib":"Beam settings",
                       "numil":"Beam settings",
                       "linjc":"LI settings",
                       "pdstl":"LI settings"}
    ...
    def _GetLastLog(self):
        return sorted(glob.glob("{0}/*_CurrentRun.log".format(LOGDIR)),reverse=True)[0]
    def GetNewRuns(self,any_subrun=False):#Run through each file in swap, say if it is a new run
        swapfiles=sorted(glob.iglob('{0}/MV*RawData.dat'.format(SWAPDIR)),key=os.path.getctime)
        for f in swapfiles:
    ...
    def SetRunRef(self,NewRunNumber):
        ...
        logfile=open("{0}/{1}_CurrentRun.log".format(LOGDIR,int(time.time())),"w")
        logfile.write("Run {0}\n".format(NewRunNumber))
        logfile.close()
    def ReadRunRef(self):	
        ...
        if lognum:
            ...
            print "Could not find a run number from the logfile {0}/CurrentRun.log".format(LOGDIR)
            sys.exit(1)
    def _isNewRun(self, filename):#Sees if a file is from a new run
        RunRef=self.ReadRunRef()
        runsearch=search("{0}/MV_([0-9]+)_([0-9]+)_(\w+)_v.*_RawData.dat".format(SWAPDIR),filename)
        if runsearch:
...
def SendEmail(Address="nur@fnal.gov",nRuns=-1): 
    ...
    msg["From"] = "minerva@minerva-cr-03.fnal.gov"
    msg["To"] = Address
